...
class MedicationQueryTest(TestCase):
    """Тести фільтрації та прогнозу медикаментів"""

    def setUp(self):
        """Створюємо тестові медикаменти з last_updated"""
        today = timezone.now()
        for i in range(5):
            Medication.objects.create(
                name=f"Med {i}",
    ...
    def test_medication_forecast(self):
        """Тест прогнозування: завжди повертає 2 місяці"""
