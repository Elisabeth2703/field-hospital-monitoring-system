...
class MongoDBManager:
  ...
  def search_medications(self, search_term):
    return list(self.medications.find({'$or': [
  ...
  def search_equipment(self, search_term):
    return list(self.equipment.find({'$or': [
      {'name': {'$regex': search_term, '$options': 'i'}},
  ...
  def bulk_update_medication_quantity(self, updates):
    operations = [
      {'update_one': {'filter': {'barcode': u['barcode']}, 'update': {'$inc': {'quantity': u['quantity_change']}, '$set': {'last_updated': datetime.now()}}}}
      for u in updates
    ]
    if operations:
