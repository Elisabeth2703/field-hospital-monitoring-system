...
from .analytics import equipment_status_analysis, low_quantity_equipment, equipment_forecast
...
def medication_detail(request, barcode):
  try:
    ...
    for log in logs:
      log['_id'] = str(log['_id'])
    context = {'medication': medication, 'logs': logs, 'usage_trend': json.dumps(usage_trend)}
...
def medication_create(request):
  if request.method == 'POST':
    try:
      ...
      db_manager.create_medication(medication_data)
  ...
  if request.method == 'POST':
    try:
      ...
      if quantity_change != 0:
        ...
        db_manager.log_medication_action(
          barcode, action, quantity_change,
          user=request.user.username if request.user.is_authenticated else 'anonymous',
